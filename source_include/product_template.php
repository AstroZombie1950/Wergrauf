<?php
/* product_template.php — общий шаблон карточки товара
...
// --- Утилиты ---
function pt_h(string $s): string {
	return htmlspecialchars($s, ENT_QUOTES, 'UTF-8');
}

function pt_price(int $price): string {
	return number_format($price, 0, '.', ' ') . ' ₽';
}

// --- Парсинг slug ---
function pt_parse_slug(string $slug): array {
	$color_suffixes = ['gm', 'gb', 'gw', 'ch', 'gr', 'b', 'g', 's', 'w'];
	$dash = strpos($slug, '-');
	if ($dash === false) return ['base' => $slug, 'size' => '', 'color' => ''];

	$base       = substr($slug, 0, $dash);
	$size_color = substr($slug, $dash + 1);
...
function pt_color_info(string $code): array {
	$map = [
		...
		'gm' => ['name' => 'Золото матовое', 'hex' => '#b8962e'],
		'gb' => ['name' => 'Золото/Черный',  'hex' => 'linear-gradient(135deg,#c9a24d 50%,#1f1f1f 50%)'],
		'gw' => ['name' => 'Золото/Белый',   'hex' => 'linear-gradient(135deg,#c9a24d 50%,#f0f0f0 50%)'],
	];
	return $map[$code] ?? ['name' => 'Комбинированный', 'hex' => 'linear-gradient(135deg,#cfcfcf,#c9a24d)'];
}

function pt_size_label(string $size): string {
...
// --- Перелинковка ---
$parsed = pt_parse_slug($slug);

$same_model = array_filter($products, fn($p) =>
	pt_parse_slug($p['slug'] ?? '')['base'] === $parsed['base']
);

// Размеры для текущего цвета (только если $has_sizes)
$sizes_for_color = [];
if ($has_sizes) {
	foreach ($same_model as $p) {
		$pp = pt_parse_slug($p['slug']);
...
// Цвета для текущего размера
$colors_for_size = [];
foreach ($same_model as $p) {
	$pp = pt_parse_slug($p['slug']);
	if ((!$has_sizes || $pp['size'] === $parsed['size']) && $pp['color']) {
...
$canonical = 'https://wergrauf.ru' . $section_url . pt_h($slug) . '/';

// Галерея
$gallery = $product['gallery'] ?? [];
if (!empty($product['image'])) array_unshift($gallery, $product['image']);
...
<head>
	...
	<meta property="og:title" content="<?= pt_h($meta_title) ?>">
	<meta property="og:description" content="<?= pt_h(mb_strimwidth($meta_desc, 0, 160, '…')) ?>">
	...
	<script type="application/ld+json">
	{
		...
		"@type": "Product",
		"name": "<?= pt_h($product['name']) ?>",
		"description": "<?= pt_h(addslashes($product['description'] ?? '')) ?>",
		"image": "<?= pt_h($og_image) ?>",
		"sku": "<?= pt_h((string)$product['article']) ?>",
		"brand": {"@type": "Brand", "name": "WERGRAUF"},
		"offers": {
			"@type": "Offer",
			"price": "<?= $product['price'] ?>",
			"priceCurrency": "RUB",
	...
	<script type="application/ld+json">
	{
		...
		"itemListElement": [
			...
			{"@type": "ListItem", "position": 2, "name": "<?= pt_h($section_name) ?>", "item": "https://wergrauf.ru<?= $section_url ?>"},
			{"@type": "ListItem", "position": 3, "name": "<?= pt_h($product['name']) ?>", "item": "<?= $canonical ?>"}
		]
	}
	</script>
	<style>
	*, *::before, *::after { box-sizing: border-box; }
	...
	.product-subinfo { display: flex; gap: 12px; align-items: center; margin-bottom: 16px; flex-wrap: wrap; }
	.product-article-info { font-size: 13px; color: #6b6b6b; }

	.product-price-box { position: relative; background: #f5f5f5; border-radius: 10px; padding: 14px 16px 14px 20px; margin-bottom: 16px; }
	.product-price-box::before { content: ""; position: absolute; left: 0; top: 8px; bottom: 8px; width: 4px; background: #9e9e9e; border-radius: 4px; }
	...
	.product-coupon { position: relative; background: #eaf6ec; border-radius: 10px; padding: 14px 16px 14px 20px; margin-bottom: 20px; }
	.product-coupon::before { content: ""; position: absolute; left: 0; top: 8px; bottom: 8px; width: 4px; background: #4caf50; border-radius: 4px; }
	...
	.color-dot { display: block; width: 28px; height: 28px; border-radius: 50%; border: 2px solid #e0e0e0; transition: border-color 0.15s; }
	.color-option input:checked + .color-dot { border-color: #2b2f33; box-shadow: 0 0 0 2px #2b2f33 inset; }
	.color-name { position: absolute; bottom: -18px; left: 50%; transform: translateX(-50%); font-size: 10px; color: #6b6b6b; white-space: nowrap; pointer-events: none; opacity: 0; transition: opacity 0.15s; }
	.color-option:hover .color-name { opacity: 1; }

	.option-sizes { display: flex; gap: 8px; flex-wrap: wrap; }
	.size-option { cursor: pointer; }
	.size-option input { display: none; }
	.size-option span { display: block; padding: 7px 12px; font-size: 13px; border-radius: 8px; border: 1px solid #dcdcdc; background: #fff; transition: all 0.15s; }
	.size-option input:checked + span { background: #2b2f33; color: #fff; border-color: #2b2f33; }

	.product-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin: 20px 0 24px; }
	.product-btn { height: 52px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; background: #2b2f33; color: #fff; border: none; transition: background 0.2s; font-family: inherit; }
	...
	.product-description { grid-area: desc; padding: 20px; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.08); }
	.product-description__title { font-size: 16px; font-weight: 600; text-transform: uppercase; margin: 5px 0; }
	.product-description__content { font-size: 14px; color: #4f4f4f; line-height: 1.6; overflow: hidden; transition: max-height 0.3s ease; }
	.product-description__content.is-collapsed { max-height: 48px; }
	.product-description__content:not(.is-collapsed) { max-height: 1000px; }
	.product-description__toggle { margin-top: 8px; padding: 0; background: none; border: none; font-size: 14px; color: #36393e; cursor: pointer; text-decoration: underline; font-family: inherit; }
	...
	.spec-row { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; border-bottom: 1px solid #e6e6e6; }
...
<div class="page-wrapper">
	...
	<section class="product-hero">
		<div class="product-hero__wrapper container">
			<nav class="product-breadcrumbs">
				<a href="/">Главная</a>
				<span>—</span>
				<a href="<?= $section_url ?>"><?= pt_h($section_name) ?></a>
				<span>—</span>
				<span><?= pt_h($product['name']) ?></span>
			</nav>
			...
			<div class="product-hero__grid">
				...
				<div class="product-card product-card--info">
					<h1 class="product-title"><?= pt_h($product['name']) ?></h1>
					<div class="product-subinfo">
						<span class="product-article-info">Арт. <?= pt_h((string)$product['article']) ?></span>
					</div>

					<!-- Цена -->
					<div class="product-price-box">
						<div class="price-main">
							<span class="price-current"><?= pt_price($product['price']) ?></span>
							<?php if ($discount_pct > 0): ?>
								<span class="price-old"><?= pt_price($product['old_price']) ?></span>
							<?php endif ?>
						</div>
						<?php if ($discount_pct > 0): ?>
							<div class="price-economy">Вы экономите <?= pt_price($product['old_price'] - $product['price']) ?></div>
						<?php endif ?>
					</div>
					...
					<div class="product-option">
						...
						<div class="option-colors">
							<?php foreach ($colors_for_size as $code => $cslug): ?>
								<?php $ci = pt_color_info($code); ?>
								<label class="color-option">
									<input type="radio" name="color" value="<?= pt_h($code) ?>"
										<?= $code === $parsed['color'] ? 'checked' : '' ?>
										onchange="window.location='<?= $section_url ?><?= pt_h($cslug) ?>/'">
									<span class="color-dot" style="background: <?= pt_h($ci['hex']) ?>"></span>
									<span class="color-name"><?= pt_h($ci['name']) ?></span>
								</label>
							<?php endforeach ?>
						</div>
					</div>
					...
					<!-- Размеры (только для душевых систем) -->
					<?php if ($has_sizes && count($sizes_for_color) > 1): ?>
					<div class="product-option">
						<div class="option-title">Размер душа</div>
						<div class="option-sizes">
							<?php foreach ($sizes_for_color as $size_code => $sslug): ?>
								<label class="size-option">
									<input type="radio" name="size" value="<?= pt_h($size_code) ?>"
										<?= $size_code === $parsed['size'] ? 'checked' : '' ?>
										onchange="window.location='<?= $section_url ?><?= pt_h($sslug) ?>/'">
									<span><?= pt_h(pt_size_label($size_code)) ?></span>
								</label>
							<?php endforeach ?>
						</div>
					</div>
					...
					<div class="product-features">
						<?php foreach (array_slice($product['specs'] ?? [], 0, 4) as $spec): ?>
							<div class="feature-card">
								<strong><?= pt_h($spec['label']) ?></strong>
								<span><?= pt_h($spec['value']) ?></span>
							</div>
						<?php endforeach ?>
					</div>
					...
					<div class="product-trust">
						<?php if ((int)($product['stock'] ?? 0) > 0): ?>
						<div class="trust-item"><strong>В наличии</strong><span><?= (int)$product['stock'] ?> шт.</span></div>
						<?php else: ?>
						<div class="trust-item"><strong>Нет в наличии</strong><span>Уточните у менеджера</span></div>
						<?php endif ?>
						<div class="trust-item"><strong>Быстрая доставка</strong><span>По всей России</span></div>
						<div class="trust-item"><strong>Официальная гарантия</strong><span>от производителя</span></div>
					</div>
				</div>
	...
	<section class="product-specs">
		<div class="product-specs__wrapper container">
			...
			<div class="product-specs__table">
				<?php foreach (array_chunk($product['specs'], 2) as $pair): ?>
					<div class="spec-row">
						<div class="spec-name"><?= pt_h($pair[0]['label']) ?></div>
						<div class="spec-value"><?= pt_h($pair[0]['value']) ?></div>
						<?php if (isset($pair[1])): ?>
							<div class="spec-name"><?= pt_h($pair[1]['label']) ?></div>
							<div class="spec-value"><?= pt_h($pair[1]['value']) ?></div>
						<?php else: ?>
							<div></div><div></div>
						<?php endif ?>
					</div>
		...
		<div class="similar-products__wrapper container">
			...
			<div class="similar-products__list">
				<?php foreach ($similar as $sp):
					$sp_disc = (!empty($sp['old_price']) && $sp['old_price'] > $sp['price'])
						? round((1 - $sp['price'] / $sp['old_price']) * 100) : 0;
				?>
					<a href="<?= $section_url ?><?= pt_h($sp['slug']) ?>/" class="similar-product-card">
						<div class="similar-product-card__image">
							<?php if ($sp_disc > 0): ?>
						...
						<div>
							...
							<div class="similar-product-card__price"><?= pt_price($sp['price']) ?></div>
						</div>
					</a>
				<?php endforeach ?>
			</div>
	...
	<section class="cooperation">
		<div class="cooperation__wrapper container">
			...
			<p class="cooperation__subtitle">Мы сотрудничаем с компаниями, дизайнерами и мастерами. Узнайте о преимуществах работы наших корпоративных программ подробнее.</p>
			<div class="cooperation__features">
				<div class="cooperation-card"><strong>Широкий ассортимент</strong><span>Все для ванной комнаты в одном месте</span></div>
				<div class="cooperation-card"><strong>Скидки и бонусы</strong><span>Специальные условия для партнеров</span></div>
				<div class="cooperation-card"><strong>Прямые поставки</strong><span>Без посредников, лучшие цены</span></div>
				<div class="cooperation-card"><strong>Актуальное наличие</strong><span>Точная информация о товарах на складе</span></div>
			</div>
